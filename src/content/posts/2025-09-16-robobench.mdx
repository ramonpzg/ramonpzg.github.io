...
import RobobenchArch from '../../../public/images/robobench1.svg'
import RobobenchArch2 from '../../../public/images/robobench2.svg'

Since the beginning of 2025, I have been working at Menlo Research (formerly Homebrew Labs) as a
research engineer and have had the chance to work on a few cool projects like `robobench`, a benchmarking
tool for our now archived wrapper around llama.cpp, [Cortex.cpp](https://github.com/menloresearch/cortex.cpp). The
timing was interesting as LLMs had become the new gold rush, and with everyone claiming they
had the fastest inference, the lowest latency, or the most efficient memory usage on their bazillion-dollar
GPU set up, we were just trying to measure what your computer said about a llama here and ther. Also,
if you hang around X long enough, you'll start to notice that most benchmark claims are about as
reliable as a weather forecast in tornado season.

The goal of our llama.cpp wrapper was to run LLMs inside hardware that could be used to control
a robot either within it or because we attached it to the robot like a fitbit or a whoop these days. This
required heavy testing on all the Pis possible (never got to an arduino), which led to my work on
`robobench`: a tool designed to measure a myriad of metrics around local deployment. While we neve made it
public, the insights and methodology behind `robobench` remain relevant for anyone serious about
measuring local LLM performance. So, in this blog post, I will walk through the architectural decisions I
made, some of the metrics I measured, and the lessons learned.

At a high level, here is a diagram of the structure of the CLI and its commands.

<RobobenchArch2 />

## What was getting measured

The most basic metrics I wanted to measure per model at the beginning were around initialisation,
runtime performance, and resources used. After setting up the foundation, I moved on to add metrics around
different integrations, workloads, quantisation levels, and scalability, as well as some advanced ones
around concurrency, GPU sharing, and resource contention.

The initial three (initialisation, runtime performance and resource usage) provided answers to questions
such as: how fast can we load a model on X device, how quickly can it generate the first token, what's the
CPU and GPU usage under regular and heavy load, and what's the memory usage under different conditions.

`robobench` focused specifically on Cortex.cpp, which was our attempt to create "the brain of robots." The
benchmarking challenge was incredibly interesting as Cortex.cpp was essentially an elegant wrapper around
llama.cpp but with a Docker-inspired CLI, integrated OpenAI-compatible API with [Scalar docs](https://scalar.com/),
and other niceties.

When you run `robobench "llama3.2:3b-gguf-q2-k" --type init`, you were running the initialisation module so
robobench would measure that loading this 3GB model spiked memory by 4.2GB (the difference between peak
and initial), takes 12.3 seconds cold versus 2.1 seconds warm, and highlighted that your "fast" SSD is
the bottleneck at 180MB/s sustained read. These were the measurements of what your hardware does when asked
to wake up an AI model.
...
The codebase is organized into distinct benchmarking modules, which means you can pick and choose 
the metrics that matter most for your use case, rather than drowning in a sea of irrelevant 
info/metadata that can take time to process (e.g., running benchmarks on a >14B model will take 
much longer than on a smaller model).
...
The cold start measurement captures the model pipeline reading the GGUF file from disk, 
allocating memory buffers, initializing the computational graph, and warming up the inference 
engine. The warm start measurement, by contrast, measures how quickly the system can reload 
an already-cached model, which is incredibly important if you only have space for one loaded 
model at a time.

What makes this genuinely useful is the memory spike calculation 
(`memory_spike = peak_memory.ram_used - initial_memory.ram_used`). When you see that loading 
llama3.2:3b requires 4.2GB of RAM despite being a 3GB model file, you get the gist of what your 
deployment needs to account for, for example, initialization overhead or memory fragmentation. 
This is why containers running "3GB models" crash with "out of memory" errors on 4GB systems.

### Runtime Performance: Beyond Tokens Per Second

The `RuntimeBenchmark` class shows what affects inference performance while the model is 
